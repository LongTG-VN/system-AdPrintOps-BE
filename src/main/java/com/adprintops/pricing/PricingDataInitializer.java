...
public class PricingDataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        // Always ensure DECAL materials match requirement (Decal đục = Decal trong + Decal UV)
        List<PricingMaterial> decalMaterials = pricingMaterialRepository.findByCategoryCodeAndActiveTrue("DECAL");
        if (decalMaterials.size() < 3 || decalMaterials.stream().anyMatch(m -> m.getMultiplier().compareTo(BigDecimal.ONE) > 0)) {
